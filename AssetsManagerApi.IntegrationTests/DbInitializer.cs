...
public class DbInitializer(CosmosDbContext dbContext)
{
    ...
    public async Task InitializeCodeAssetsAsync()
    {
        ...
        var notCombinedStartProject = new StartProject
        {
            Id = "b3ceafbb-9c1f-4d2d-9e8a-ffb0f688fdc4",
            CodeAssetsIds = [..codeAssets.Select(x => x.Id)],
            CompanyId = digitalBank.Id,
            CreatedDateUtc = DateTime.UtcNow,
            CreatedById = startProjectUser.Id
    ...
    public async Task InitializeCodeAssetsManuallyAsync()
    {
        ...
        var startProjectUser = usersCollection.GetItemLinqQueryable<User>(allowSynchronousQueryExecution: true)
                                            .Where(u => u.Id == "d3aeadbb-9c1f-4d2d-9e8a-ffb0f688fdc4")
                                            .AsEnumerable()
                                            .FirstOrDefault();
